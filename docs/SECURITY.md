...
| 球員（自己註冊） | 上述全部 + 回覆練球會到／小遲／下次一定、開練球通知 | 他自己在「練球」頁註冊，註冊時選名單上的名字 | 「編輯名單」的「練球帳號」欄解除綁定；帳號留著也寫不了任何比賽資料 |
...
球員註冊的信箱存在 `player_accounts`，RLS 設成只有本人和紀錄員讀得到，公開的球員名單不含信箱。註冊時的「認領名字」由資料庫函式 `claim_player_name()` 檢查：名字必須在名單上、還沒被別人註冊，一個帳號也只能對到一個名字。認領成功也只換到「回覆自己的練球」這一件事。

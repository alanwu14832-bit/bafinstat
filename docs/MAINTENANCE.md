...
| **球員（練球投票）** | 約 40 | 自己註冊帳號（選名單上的名字）後，可回覆練球「會到／小遲／下次一定」、開練球通知；**註冊不會取得紀錄員權限** | 自己註冊一次；練球前一天收到通知就回覆；iPhone 要先把網站「加入主畫面」才收得到通知 |
...
球員不需要管理員開帳號，自己在網站「練球」頁按「註冊」：填自己的 email、設一組密碼、**從球員名單的下拉選單選自己的名字**，帳號就跟名單上那個人綁在一起。管理員只要確保那個人已經在「球員」頁的名單裡。

- 註冊完全不會拿到紀錄員權限。能寫比賽資料的仍然只有 `editors` 表裡的信箱；球員只能改自己的那一票。
- 一個名字只能被註冊一次，一個帳號也只能對到一個名字。名字被別人搶註冊、或有人用錯名字時，管理員到「球員」頁 →「編輯名單」，在「練球帳號」欄按解除綁定的圖示，那個名字就能重新註冊。
- 名單上沒有他的名字時，選單裡就選不到。先把人加進名單再請他註冊。
- Supabase 那邊要開著 Authentication → Providers → Email 的 `Enable email signups`（一次性設定，見 `docs/SUPABASE_SETUP.md`）。

球員的信箱存在 `player_accounts`，只有本人和紀錄員看得到，不會出現在公開的球員名單裡。
...
| 有人換信箱／搞錯名字 | 管理員 | 「球員」頁 → 編輯名單 → 「練球帳號」欄解除綁定，請他重新註冊 |

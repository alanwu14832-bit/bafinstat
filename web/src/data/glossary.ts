...
const H: Record<string, Hint> = {
  ...
  '近 5 場': { title: '近五場戰績', text: '目前篩選範圍內最近五場的勝敗與得失分，看球隊近況。' },
  每場失分: { title: '每場失分', text: '平均每場被對手得幾分，投手與守備合起來的結果。' },
  每場殘壘: { title: '每場殘壘', text: '平均每場有幾個跑者上壘後沒有回來得分；高代表打線串不起來。' },
  每場失誤: { title: '每場失誤', text: '平均每場守備失誤次數；業餘比賽的勝負常常就差在這裡。' },
  'BB% / K%': { title: '保送率／三振率', text: '打者每個打席選到保送、被三振的比率；選球好、少揮空的打線這兩個數字一低一高。' },
  '戰績（勝-敗-和）': { title: '戰績', text: '目前篩選範圍內的勝、敗、和場數。' },
  得失分差: { title: '得失分差', text: '總得分減總失分，正數代表淨勝分。' },
  每場得分: { title: '每場得分', text: '平均每場比賽得幾分。' },
  每場得失分: { title: '每場得分／失分', text: '平均每場得幾分、被得幾分；前面是我隊得分，後面是失分。' },
  '每場失誤 / 守備率': { title: '每場失誤／守備率', text: '平均每場失誤次數，以及（刺殺＋助殺）÷ 守備機會的成功率。' },

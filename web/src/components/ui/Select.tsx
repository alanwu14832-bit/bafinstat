...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(function Select({ label, options, size = 'md', className, id, ...rest }, ref) {
  ...
  return (
    <label
      htmlFor={selectId}
      className={cx(
        'press relative inline-flex items-center bg-surface-3/60 border border-transparent rounded-[10px] text-ink min-w-0 cursor-pointer',
        ...
        size === 'sm' ? 'h-9 pointer-fine:h-8 text-[13px]' : 'h-10 pointer-fine:h-9 text-sm',
        className,
      )}
    >

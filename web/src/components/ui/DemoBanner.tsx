...
export function DemoBanner() {
  ...
  return (
    <div role="status" className="flex items-center gap-3 rounded-[var(--radius-sm)] border border-border bg-surface-2 px-3 py-2 text-[13px] text-ink-2">
      ...
      <button type="button" onClick={() => setDemo(false)} className="ml-auto inline-flex items-center gap-1 h-9 px-2.5 pointer-fine:h-7 pointer-fine:px-2 rounded-[6px] text-ink-2 hover:bg-surface-3 hover:text-ink cursor-pointer whitespace-nowrap text-xs font-medium">

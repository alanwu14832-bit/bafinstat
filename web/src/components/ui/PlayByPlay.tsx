...
/** Pitch code → colour chip. 好球類：S/SS/CS/IP；界外 F；壞球 B */
const PITCH_STYLE: Record<string, { label: string; cls: string; title: string }> = {
  SS: { label: 'SS', cls: 'bg-[color-mix(in_srgb,var(--series-8)_22%,transparent)] text-ink', title: '揮棒落空' },
  CS: { label: 'CS', cls: 'bg-[color-mix(in_srgb,var(--series-1)_22%,transparent)] text-ink', title: '未揮棒好球' },
  S: { label: 'S', cls: 'bg-[color-mix(in_srgb,var(--series-1)_22%,transparent)] text-ink', title: '好球' },
  F: { label: 'F', cls: 'bg-[color-mix(in_srgb,var(--series-4)_28%,transparent)] text-ink', title: '界外' },
  IP: { label: 'IP', cls: 'bg-[color-mix(in_srgb,var(--series-3)_24%,transparent)] text-ink', title: '擊進場內' },
...
const CODE_LABEL: Record<string, string> = { I: '1出局', II: '2出局', III: '3出局', L: '殘壘', R: '得分', ER: '自責分' }
const codeBadge = (code?: string) => {
  if (!code) return null
  const out = code === 'I' || code === 'II' || code === 'III'
  const run = code === 'R' || code === 'ER'
  return <span className={cx('inline-flex h-5 px-1.5 rounded-full text-[11px] font-medium', run ? 'bg-[color-mix(in_srgb,var(--good)_18%,transparent)] text-ink' : out ? 'bg-surface-3 text-ink-2' : 'bg-accent-soft text-ink')}>{CODE_LABEL[code] ?? code}</span>
}

const resultCls = (r: string) => (isHitResult(r) ? 'font-semibold text-ink' : r === '保送' || r === '故四' || r === '觸身' ? 'text-ink' : r === '三振' ? 'text-ink-2' : 'text-ink-2')
...
function InningHeader({ inning, half }: { inning: number; half: string }) {
  return (
    <tr>
      <td colSpan={9} className="px-3 pt-3 pb-1 text-[11px] font-semibold tracking-wider text-muted uppercase">第 {inning} 局{half}</td>
    </tr>
  )
}

const th = 'px-3 py-2 text-left text-[11px] font-medium text-muted whitespace-nowrap'
...
export function BattingPlayByPlay({ pas }: { pas: BattingPA[] }) {
  if (!pas.length) return <div className="text-sm text-muted px-3 py-6 text-center">沒有逐打席紀錄</div>
  let lastInning = 0
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm border-collapse min-w-[720px]">
        <thead className="sticky top-0 bg-surface">
          <tr><th className={th}>局</th><th className={th}>棒次</th><th className={th}>打者</th><th className={th}>逐球</th><th className={th}>球數</th><th className={th}>結果</th><th className={th}>擊球</th><th className={th}>跑壘</th><th className={th}>狀態</th></tr>
        </thead>
            ...
            return (
              <Fragment key={i}>
                ...
                <tr className="border-t border-border hover:bg-surface-2">
                  <td className={cx(td, 'text-muted')}>{p.outsBefore !== undefined ? `${p.outsBefore} 出局` : ''}{p.basesBefore && p.basesBefore !== '無' ? `・壘上 ${p.basesBefore}` : ''}</td>
                  ...
                  <td className={td}>{codeBadge(p.code)}{p.note && <div className="text-[11px] text-muted mt-0.5">{p.note}</div>}</td>
                </tr>
              </Fragment>
            )
          })}
...
export function PitchingPlayByPlay({ pas }: { pas: PitchingPA[] }) {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm border-collapse min-w-[720px]">
        <thead className="sticky top-0 bg-surface">
          <tr><th className={th}>局</th><th className={th}>對方棒次</th><th className={th}>投手</th><th className={th}>逐球</th><th className={th}>球數</th><th className={th}>結果</th><th className={th}>擊球</th><th className={th}>跑壘／守備</th><th className={th}>狀態</th></tr>
        </thead>
        <tbody className="tnum">
          {pas.map((p, i) => {
            const pt = pitchTotals(p.pitches)
            ...
            return (
              <Fragment key={i}>
                ...
                <tr className={cx('border-t border-border hover:bg-surface-2', changed && 'border-t-2 border-t-[color-mix(in_srgb,var(--accent)_50%,transparent)]')}>
                  <td className={cx(td, 'text-muted')}>{p.outsBefore !== undefined ? `${p.outsBefore} 出局` : ''}{p.basesBefore && p.basesBefore !== '無' ? `・壘上 ${p.basesBefore}` : ''}</td>
                  <td className={td}>{p.oppOrder ?? ''}{p.oppBatter ? <span className="text-muted text-xs ml-1">{p.oppBatter}</span> : null}</td>
                  <td className={cx(td, 'font-medium whitespace-nowrap')}>{p.pitcher}{changed && <span className="text-[10px] text-accent ml-1">換投</span>}</td>
                  <td className={td}><PitchChips pitches={p.pitches} /></td>
                  <td className={cx(td, 'text-muted whitespace-nowrap')}>{pt.pitches} 球・{pt.strikes}S {pt.balls}B</td>
                  <td className={cx(td, resultCls(p.result))}>{p.result || '—'}</td>
                  <td className={cx(td, 'text-ink-2 whitespace-nowrap')}>{hitLoc(p.loc, p.traj, p.quality) || '—'}</td>

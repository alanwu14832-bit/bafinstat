...
export function DonutCard({ segments, centerValue, centerCaption, size = 200, formatValue, ...card }: DonutCardProps) {
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-center gap-6 min-w-0">
          <div className="relative shrink-0" style={{ width: size, height: size }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <div className="font-display font-bold text-[32px] leading-none text-ink">{centerValue ?? fmt(total)}</div>
              {centerCaption && <div className="text-xs text-muted mt-1">{centerCaption}</div>}
            </div>
          </div>
          <ul className="flex flex-col gap-2 w-full sm:w-auto sm:flex-1 sm:min-w-[140px] min-w-0 text-sm">
            {data.map((d) => (

...
const FAQ: Array<[string, string]> = [
  ...
  ['練球怎麼回覆、怎麼點名？', '「練球」頁：球員第一次先按「註冊」，填自己的 email、設密碼、從名單選自己的名字（註冊不會拿到紀錄員權限），之後按會到／小遲／下次一定，下次一定可填請假原因（只有管理員看得到）；截止是練球當天 09:00，之後仍可改但算晚回。按「開啟通知」練球前一天 18:00 會推播（iPhone 要先用 Safari「加入主畫面」）。紀錄員在「管理」分頁設每週固定練球與停練期間（考試週自動含考前一週），練球當天展開那一場「依投票預填點名」再改沒來的人；出席率以點名為準，只有紀錄員看得到出席率。'],

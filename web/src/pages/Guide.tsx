...
const FAQ: Array<[string, string]> = [
  ...
  ['賽程怎麼排？', '紀錄員登入後到「賽程」頁「新增賽程」填日期、時間、對手、主客、場地與集合備註。隊員看得到接下來的比賽並可加到 Google 日曆。比賽當天紀錄員在「紀錄比賽」用「從賽程帶入」選這場，資料自動填好、比賽ID 沿用。排定但還沒記錄的比賽不會算進任何統計。'],
  ['練球怎麼回覆、怎麼點名？', '「練球」頁：球員用名單上的信箱登入（登入不會拿到紀錄員權限），按會到／小遲／下次一定，下次一定可填請假原因（只有管理員看得到）；截止是練球當天 09:00，之後仍可改但算晚回。按「開啟通知」練球前一天 18:00 會推播（iPhone 要先用 Safari「加入主畫面」）。紀錄員在「管理」分頁設每週固定練球與停練期間（考試週自動含考前一週），練球當天展開那一場「依投票預填點名」再改沒來的人；出席率以點名為準，只有紀錄員看得到出席率。'],

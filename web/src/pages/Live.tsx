...
import { BOARD, BoardTray, CountLights, LineScoreBoard } from '../components/ui/Scoreboard'
...
export function LivePage() {
  ...
  return (
    <>
      ...
      <BoardTray className="rounded-[calc(var(--radius)+5px)]" innerClassName="rounded-[var(--radius)]">
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4 md:gap-8 text-center p-5 md:p-8">
          <div><div className="text-[13px] md:text-[15px] truncate" style={{ color: BOARD.muted }}>{weTop ? TEAM_NAME : s.game.opponent}</div><div className="figure text-[56px] md:text-[84px] font-semibold leading-none mt-1"><RollingNumber text={String(weTop ? sc.us : sc.opp)} /></div></div>
          ...
          <div><div className="text-[13px] md:text-[15px] truncate" style={{ color: BOARD.muted }}>{weTop ? s.game.opponent : TEAM_NAME}</div><div className="figure text-[56px] md:text-[84px] font-semibold leading-none mt-1"><RollingNumber text={String(weTop ? sc.opp : sc.us)} /></div></div>
        </div>
        <LineScoreBoard bare className="border-t" innings={n} current={{ inning: s.inning, half: s.half }} showErrors={false}
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-5">
        <Card title={side === 'us' ? '我隊最近打席' : '對方最近打席'} flush>
          <ul className="divide-y divide-[var(--border)] text-[13px]">
            ...
            {/* A new plate appearance slides in at the top; the older ones make room. Low frequency, so it earns the motion. */}

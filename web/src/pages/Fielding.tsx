...
export function FieldingPage() {
  ...
  const columns: Column<FieldingStat>[] = [
    ...
    { key: 'positions', header: '守位', format: (v) => (v as string[]).join(' / ') },
  ...
  return (
    <>
      <PageHeader eyebrow="Fielding" title="守備" description="守備紀錄以每場每位球員一列；守位篩選會直接套用在此頁。" />
      <DemoBanner />
      ...
      <Card title="守備成績" subtitle="點選球員開啟個人檔案" flush>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
        ...
        <BarChartCard className="xl:col-span-3" title="各守位失誤" subtitle="依守位彙總" data={byPos} series={[{ key: '失誤', label: '失誤' }]} highlightKey={byPos.reduce((m, p) => (p.失誤 > (byPos.find((x) => x.name === m)?.失誤 ?? -1) ? p.name : m), '')} />
      </div>
      {catchers.length > 0 && (
        <Card title="捕手" subtitle="被盜壘、阻殺與捕逸">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {catchers.map((c) => (
              <div key={c.name} className="rounded-[var(--radius-sm)] border border-border p-3">
                <div className="font-medium text-ink">{c.name}</div>
                <div className="text-xs text-muted mt-1 tnum">被盜 {c.sb}・阻殺 {c.cs}・捕逸 {c.pb}</div>
                <div className="font-display font-bold text-[24px] text-ink mt-1">{pct(c.csPct)} <span className="text-xs text-muted font-body font-normal">CS%</span></div>
              </div>
            ))}
          </div>
        </Card>

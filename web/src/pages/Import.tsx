  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
        <Card className="xl:col-span-3" title="上傳檔案" subtitle="支援總表或單場模板（.xlsx / .xlsm）">
          <div role="button" tabIndex={0} aria-label="拖曳檔案到此處或點選上傳" onClick={() => inputRef.current?.click()} onKeyDown={(e) => e.key === 'Enter' && inputRef.current?.click()}
            ...
            <div className="text-xs text-muted">會辨識「打席紀錄／投球紀錄／守備紀錄」或「單場-打擊／單場-投球」工作表</div>
          ...
          {pending && (
            <div className="mt-4 rounded-[var(--radius-sm)] border border-border p-4 flex flex-col gap-3">
              <div className="flex items-center gap-2 flex-wrap"><Badge variant="accent">{pending.report.mode === 'master' ? '總表' : '單場模板'}</Badge><span className="text-sm font-medium text-ink">{pending.file}</span></div>

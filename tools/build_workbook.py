...
lines = [
    ...
    ("打擊結果", True),
    ("一安 二安 三安 全壘打 保送 故四(故意四壞) 觸身 三振(含不死三振) 內滾 內飛 外飛 野選 失誤(失誤上壘) 犧觸 犧飛 雙殺 妨礙(捕手妨礙)", False),
    ("結果代碼（沿用原表）", True),
    ...
    ("落點：只填 1–9 的數字（1 投 2 捕 3 一壘 4 二壘 5 三壘 6 游擊 7 左 8 中 9 右）。軌跡 G 滾地／F 飛球／L 平飛。強度 強／中／弱 → 近似 Statcast 的 Hard-Hit%。", False),
...
def bat_formulas(rr):
    ...
    H = c("慣用手")
    f["拉打"] = f'=IF({c("場內球")}=0,0,IF({H}="L",IF(OR({Y}=3,{Y}=4,{Y}=9),1,0),IF(OR({Y}=5,{Y}=6,{Y}=7),1,0)))'
    f["中間"] = f'=IF({c("場內球")}=0,0,IF(OR({Y}=1,{Y}=2,{Y}=8),1,0))'
    f["反方向"] = f'=IF({c("場內球")}=0,0,IF({H}="L",IF(OR({Y}=5,{Y}=6,{Y}=7),1,0),IF(OR({Y}=3,{Y}=4,{Y}=9),1,0)))'
